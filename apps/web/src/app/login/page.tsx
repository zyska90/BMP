...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Direct type-safe login API call using Eden Treaty
      ...
      if (status !== 200 || !data || apiErr) {
        setError(apiErr?.value?.message || 'Login failed. Please verify credentials.');
        setLoading(false);
        return;
      }

      // Successful login, redirect depending on profile status
      setLoading(false);
  ...
  return (
    <div className="relative min-h-screen bg-gray-950 text-white flex items-center justify-center p-6 select-none">
      ...
      <div className="relative w-full max-w-[420px] bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-2xl z-10">
        ...
        {/* Error message */}
        {error && (
          <div className="mb-4 p-3.5 bg-red-500/10 border border-red-500/20 text-red-500 rounded-lg text-sm font-medium animate-shake">
            ⚠️ {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 mt-2 bg-gradient-to-r from-brand-500 to-brand-700 hover:from-brand-600 hover:to-brand-800 disabled:opacity-50 text-white font-semibold rounded-lg shadow-lg shadow-brand-500/10 transform active:scale-98 transition-all duration-150 flex items-center justify-center gap-2"
          >
            {loading ? (
